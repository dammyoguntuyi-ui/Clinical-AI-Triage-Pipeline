...
def process_unified_clinical_packet(data):
    ...
    try:
        ...
        # Handle the binary pydicom parsing layer safely if present
        if b64_imaging_data:
            try:
                ...
                # Extract real attributes from the DICOM object layout
                ...
                # Dynamic inference execution from the local AI evaluation module
...
def automated_clinical_stream():
    ...
    # Check infrastructure connection before running the pipeline
        ...
        with right_layout:
            ...
            if not st.session_state.clinical_history.empty:
                if latest_entry["Has Imaging"] == "Yes":
                    ...
                    # 💡 FIX: Color code based directly on the text of the AI findings
                    is_pathology_detected = any(keyword in latest_entry["AI Radiology Findings"] for keyword in ["Hemorrhage", "Mass", "Fracture", "Abnormality"])
                    ...
                    st.warning("🔲 No diagnostic radiology exams were scheduled or ordered for this encounter window. Bedside monitoring remains ongoing.")
            else:
