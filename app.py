...
def process_unified_clinical_packet(data):
    ...
    try:
        ...
        # ENHANCED: Combine Modality and Body Site for full visibility
        has_imaging = "Yes" if imaging else "No"
        if imaging:
            modality_info = f"{imaging['modality']} ({imaging['bodySite']})"
...
if st.sidebar.button("🗑 Clear Local Log Cache", use_container_width=True):
    st.session_state.clinical_history = pd.DataFrame(
        columns=[
            "Timestamp",
            "Patient ID",
...
def automated_clinical_stream():
    ...
    if check_streamer_network_resilience():
        ...
        if not st.session_state.clinical_history.empty:
            ...
            with right_layout:
                ...
                if latest_entry["Has Imaging"] == "Yes":
                    ...
                    st.warning("🔲 No diagnostic radiology exams were scheduled or ordered for this encounter window. Bedside monitoring remains active.")
        else:
        ...
        st.warning(
            ...
            "The system network bridge is attempting automated container self-healing. Retrying connection..."
        )
